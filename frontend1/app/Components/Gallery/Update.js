...
function Update() {
  ...
  useEffect(() => {
    const fetchAlbum = async () => {
      setIsLoading(true)
      try {
        const response = await fetch(`http://localhost:5001/getalbum/${albumid}`, {
          headers: {
  ...
  const handleGalleryUpload = async () => {
    ...
    try {
      await axios.post(`http://localhost:5001/upload-gallery/${albumid}`, formData, {
  ...
  const removeImage = async (imageId) => {
    try {
      const response = await fetch(`http://localhost:5001/removeImage/${albumid}/${imageId}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
